<h1 align="center"> 🛒 백엔드 프로젝트: Twofaang 🛒</h1>
...
1. 관리자가 상품 등록 시 상품의 상태를 '대기'로 놓고 서비스 시작 시 등록이 되도록
(실수로 금액이나 다른 정보들을 잘 못 등록했을 시 회원이 구매하는 것을 방지하기 위함)

2. 관리자가 상품 삭제 시 바로 삭제 하지 말고 상태값을 바꿔서 상품의 정보를 확인 할 수 있도록 해야 함

3. api명세서 작성 시 url에 id나 dto가 필요할 시 해당 dto나 id에 대한 설명도
옆에 명시해주면 좋을 것 같음

4. 스케줄러를 사용하였는데 스프링 배치를 사용하는 것을 고려해 볼 것

5. 비밀번호가 너무 짧으면 경고창 띄우게 하고 더미데이터로 테스트 할때도 짧게도 길게도 시도해 볼 것
